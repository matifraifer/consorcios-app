...
export default function MpCallback() {
  ...
  useEffect(() => {
    ...
    supabase.functions
      .invoke('mp-auth', { body: { code, redirect_uri: REDIRECT_URI, cliente_id: clienteId } })
      .then(({ data, error: fnError }) => {
        if (fnError) throw fnError
        if (data?.error) throw new Error(JSON.stringify(data.error))
        setStatus('success')
        setTimeout(() => navigate('/configuracion'), 2500)
      })
      .catch(err => {
      ...
      <Box sx={{
        ...
        {status === 'loading' && (
          <>
            ...
            <Typography sx={{ fontWeight: 600, color: '#111827', mb: 0.5 }}>Conectando con Mercado Pago...</Typography>
