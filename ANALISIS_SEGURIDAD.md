...
## 6. RESUELTO — CORS abierto (`*`) en todas las Edge Functions

Todas las funciones (`mp-crear-preferencia`, `mp-webhook`, `ml-auth`, `enviar-*`, `extraer-contrato-ia`) devolvían `Access-Control-Allow-Origin: '*'`. Para las funciones realmente públicas (llamadas desde `PropiedadPublica.jsx`, `/consulta/:token`) tiene sentido. Para las que se llaman solo desde el panel admin logueado (o desde el login/"olvidé mi contraseña", antes de tener sesión), `*` no aportaba ninguna protección real — era defensa en profundidad, no el problema principal (ya resuelto en el punto 1), pero valía la pena cerrarla.

**Fix aplicado**: `ml-auth`, `mp-auth`, `ml-test`, `mp-test`, `enviar-link-consulta`, `enviar-liquidacion-whatsapp`, `enviar-liquidacion-email`, `extraer-contrato-ia` y `resolve-username` (9 funciones — nunca se llaman desde una página pública) reemplazan el `Access-Control-Allow-Origin: '*'` fijo por un chequeo dinámico contra un allowlist (`corsHeaders(req)`, calculado a partir del header `Origin` del request): `https://app.granito.com.ar` (dominio de producción), `https://consorcios-app.vercel.app` (URL vieja de Vercel, todavía usada como redirect fijo de MercadoLibre) y cualquier `http://localhost:*` (dev). Si el origin no matchea, se devuelve el primero del allowlist como `Allow-Origin` (el browser va a bloquear la respuesta igual, porque no coincide con el origin real de la página que hizo la llamada). `mp-crear-preferencia` (llamada desde páginas públicas sin login) y `mp-webhook` (server-to-server, lo llama Mercado Pago, no un browser) se dejan con `*` sin cambios.
