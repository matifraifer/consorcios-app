-- Comisión de la plataforma sobre los cobros de expensas por Mercado Pago.
-- Modelo: el propietario paga un recargo (comisión de MP + fee fijo de la
-- plataforma) por encima de la deuda real, calculado para que al consorcio
-- le llegue el 100% del monto de la expensa y a la plataforma le llegue el
-- fee completo vía marketplace_fee. Ver mp-crear-preferencia.

alter table public.consorcios
  add column if not exists comision_plataforma_fee numeric(12,2),
  add column if not exists permite_pagos_parciales boolean not null default true;

alter table public.mp_pagos
  add column if not exists comision_plataforma numeric(12,2),
  add column if not exists recargo_mp numeric(12,2);

-- Extiende consultar_deuda_departamento (0010, 0012) para devolver también
-- la config de cobro del consorcio, así el frontend sabe si mostrar
-- selección por período o solo el total, y si mostrar el desglose de tarifa.
