...
supabase/
  ...
  functions/                      # Edge Functions (Deno) — deployadas con --no-verify-jwt
    ...
    mp-webhook/                    # Pública — valida la firma de MP (MP_WEBHOOK_SECRET), confirma el estado real contra la API de MP y marca expensas_departamento como pagado
...
### Consorcios / expensas
- `consorcios`: id (UUID PK), nombre, cliente_id (FK clientes_servicio), tasa_mora (numeric, default 0), comision_plataforma_fee (numeric, nullable — fee fijo de la plataforma por pago vía Mercado Pago; null/0 = sin cargo), permite_pagos_parciales (bool, default true — si false, el propietario solo puede pagar el total adeudado, no períodos sueltos)
- `departamentos`: id (serial PK), numeracion, propietario (text libre, legacy, sin uso), inquilino, id_propietario (FK propietarios, legacy — ya no se escribe desde ningún formulario, se mantiene por datos históricos), propietario_nombre, propietario_apellido, propietario_dni (text, nullable — dato del propietario cargado directo en el departamento, reemplaza al desplegable de `propietarios` en los formularios de alta; misma convención que `contratos.propietario_nombre/apellido/dni`), id_consorcio (UUID FK), coeficiente, email, telefono (text, nullable — formato E.164, usado por el envío manual de liquidación por WhatsApp vía Twilio), token_consulta (UUID unique — link de consulta sin login para el propietario/inquilino), activo (boolean, default true — baja lógica: "Eliminar" en la UI nunca borra la fila, solo pone `activo=false`)
...
- `contratos`: id (UUID), cliente_id, propiedad_id (nullable — la propiedad puede vincularse después del alta), nomenclatura_catastral (text, nullable), es_compraventa (bool, default false — checkbox "Es un contrato de compraventa" en el form; cambia el texto de "Concepto" del recibo de pago de "Alquiler del mes..." a "Cuota del mes..."), inquilino_nombre/apellido/dni/telefono, propietario_nombre/apellido/dni/telefono, fecha_inicio, fecha_fin, monto_base, tipo_actualizacion, plazo_actualizacion, dia_vencimiento, observaciones, finalizado (bool), servicio_agua/servicio_gas/servicio_energia (text, nullable — número de cuenta o suministro de cada servicio, sección "Datos de servicios" del formulario), created_at, updated_at. `inquilino_telefono`/`propietario_telefono` (celular, opcional) se copian al crear/actualizar el contacto correspondiente en `contactos.telefono` (solo si ese contacto todavía no tenía teléfono cargado) vía `vincularContactoDesdeContrato`
...
- `recibos_contrato`: id (UUID), cliente_id, contrato_id, pago_id (FK `pagos_contrato`, unique — un recibo por pago), serie (text, default `'0001'`), numero (integer), fecha_pago, monto, created_at — numeración de recibo `SERIE-NUMERO` con `serie` fija en 4 dígitos y `numero` autoincremental **por cliente** (dos clientes pueden compartir el mismo número), `unique(cliente_id, serie, numero)`. Se crea automáticamente al registrar un pago (`registrarPagoContrato` en `supabase.js`, no bloquea el pago si falla) llamando a la RPC `crear_recibo_contrato`, que toma el próximo número de `recibos_numeracion` (una fila por cliente con el último número usado, actualizada con `ON CONFLICT` para que el lock de fila evite duplicados ante pagos simultáneos) e inserta el recibo, todo en una transacción. El PDF (`src/services/reciboContrato.js`, botón "Descargar recibo" en `ContratoDetalleDrawer.jsx` junto a cada período pagado) se arma en el momento consultando `recibos_contrato` — no se guarda el PDF, se regenera cada vez con jsPDF
- `recibos_numeracion`: cliente_id (PK), ultimo_numero — solo lo usa la RPC `crear_recibo_contrato`, no se lee directo desde el frontend
...
- `mp-webhook`: primero valida la firma del webhook (header `x-signature`, HMAC-SHA256 con el secret `MP_WEBHOOK_SECRET` — "Firma secreta" de la app en el panel de Mercado Pago Developers, sección Webhooks; no confundir con `MP_CLIENT_SECRET`) y corta con 401 si no matchea. Recién ahí busca `mp_pagos` por `pago_id`, y con el `access_token` del cliente dueño de ese pago confirma el estado real contra `GET /v1/payments/:id` (nunca confía en el payload del webhook a ciegas). Si `approved`, marca `mp_pagos.estado` y actualiza `expensas_departamento` (`pagado=true`) de cada período cubierto
...
- Pendiente: cargar el secret `MP_WEBHOOK_SECRET` de `mp-webhook` (ver esa función más abajo) con la "Firma secreta" de la app, panel Developers → tu app → Webhooks
...
## Integración Twilio (envío de liquidación de expensas por WhatsApp)
- Canal aparte de la integración Baileys de abajo (mensajería manual) — este es para el envío del saldo adeudado por WhatsApp, vía la API oficial de WhatsApp Business de Twilio. No hay recordatorios automáticos por cron, solo envío manual a pedido del admin (se descartó adrede la opción de recordatorios cronológicos)
...
- **Plantilla de WhatsApp (obligatoria)**: WhatsApp Business API no permite texto libre cuando la empresa inicia la conversación (fuera de la ventana de 24hs desde el último mensaje del usuario) — hay que usar una plantilla (Content Template) aprobada por Meta desde Twilio Content Template Builder. El Content SID vive en el secret `TWILIO_TEMPLATE_SID` y `enviar-liquidacion-whatsapp` manda `ContentSid`+`ContentVariables` en vez de `Body`. Plantilla actual: "Hola Vecino/a! Te enviamos el saldo de expensas impago al dia de la fecha del consorcio {{1}}. Saldo total {{2}} fecha de vencimiento del periodo {{3}}", con un botón de URL de base fija `https://app.granito.com.ar/consulta/{{4}}` (variable 4 = solo el `token_consulta`, no la URL completa). Si se edita el texto/orden de variables de la plantilla en Twilio, hay que actualizar el `contentVariables` de la función para que siga matcheando
- Cada departamento necesita `telefono` cargado (formato E.164, ej. `+5491122334455`) para poder recibir el mensaje — campo en el drawer de edición de departamento, al lado del email
...
- **Envío**: Edge Function `enviar-liquidacion-whatsapp`, siempre a demanda (nunca por cron), manda el saldo total adeudado (mismo cálculo de mora que `enviar-link-consulta`/`calcularSaldosMora.js`, duplicado en la función porque las Edge Functions no comparten bundle con el frontend) a todas las unidades del consorcio con `telefono` cargado y deuda pendiente. Se dispara desde dos lugares en `ConsorcioDetalle.jsx`: el botón "Enviar liquidación por WhatsApp" (tab Liquidaciones, junto a "Descargar PDF", con confirmación previa) y el paso 2 ("Avisar a los vecinos") del flujo de cierre de período, cuando se elige el canal WhatsApp o "ambas". Se puede reenviar las veces que haga falta, no hay deduplicación
...
- Login por **nombre de usuario** (no email) — el frontend resuelve `nombre_usuario` → `email` con la Edge Function `resolve-username` (`--no-verify-jwt`, consulta `usuarios` con la service-role key, con rate-limit propio por IP vía `resolve_username_intentos`: 20 intentos/15 min) y recién ahí llama a `supabase.auth.signInWithPassword`. La RPC vieja `email_for_username` (`0005_username_login_rpc.sql`) era un oráculo de enumeración de usuarios sin rate limit (ver `ANALISIS_SEGURIDAD.md`, punto 4) — se eliminó en `0033_resolve_username_rate_limit.sql`
...
- Bucket de Storage `propiedades-imagenes`: policy `insert/update/delete` a `authenticated` segmentada por `cliente_id` en el path (`0034_storage_aislamiento_cliente.sql`, cubre los 3 patrones: `logos/{cliente_id}/...`, `portada/{cliente_id}/...`, `{cliente_id}/{propiedad_id}/...`); lectura pública porque el bucket es público
- Buckets `contratos-adjuntos`/`documentos-respaldatorios` (usados por `CONTRATOS_BUCKET`/`DOCUMENTOS_BUCKET` en `supabase.js`, solo desde componentes de admin logueado): tenían acceso público (`anon`/`public`) sin ninguna restricción, configurado a mano en el dashboard de Supabase, no trackeado en ninguna migración — cerrado a `authenticated` en `0035_storage_cerrar_acceso_anon.sql` y aislado por `cliente_id` en `0036_storage_fase2_contratos_documentos.sql` (vía `EXISTS` contra `contratos`/`pagos_contrato`/`propiedades`, ya que estos paths no incluyen `cliente_id` directo como sí hace `propiedades-imagenes`; ver `ANALISIS_SEGURIDAD.md`, punto 5)
...
- `supabase/migrations/0015_recordatorios_whatsapp.sql`: **pendiente** — agrega `telefono` a `departamentos`, usado por el envío manual de liquidación por WhatsApp (`enviar-liquidacion-whatsapp`, secrets `TWILIO_ACCOUNT_SID`/`TWILIO_AUTH_TOKEN`/`TWILIO_WHATSAPP_NUMBER`)
...
- `supabase/migrations/0022_contrato_nc_y_servicios.sql`: **pendiente** — agrega `nomenclatura_catastral`/`servicio_agua`/`servicio_gas`/`servicio_energia`/`propietario_telefono`/`inquilino_telefono` a `contratos` y quita el `not null` de `contratos.propiedad_id`
- `supabase/migrations/0023_recibos_contrato.sql`: **pendiente** — crea `recibos_numeracion`/`recibos_contrato` + RLS + la función `crear_recibo_contrato` (numeración de recibos de pago de alquiler)
- `supabase/migrations/0024_contrato_compraventa_y_cliente_direccion.sql`: **pendiente** — agrega `contratos.es_compraventa` (default false) y `clientes_servicio.direccion`
